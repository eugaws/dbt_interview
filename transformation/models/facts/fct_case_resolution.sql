...
final as (
    select
        ...
        case
            when
                datediff('day', createddate, closeddate) > 1 and datediff('day', createddate, closeddate) <= 3
                then 1
            else 0
        end as resolution_speed_within_3_days_flg,
        case
            when
                datediff('day', createddate, closeddate) > 3 and datediff('day', createddate, closeddate) <= 7
                then 1
            else 0
        end as resolution_speed_within_1_week_flg,
        case
            when
                datediff('day', createddate, closeddate) > 7 and datediff('day', createddate, closeddate) <= 30
                then 1
            else 0
        end as resolution_speed_within_1_month_flg,
        ...
        case when not isescalated and datediff('day', createddate, closeddate) <= 7 then 1 else 0 end
            as resolution_quality_high_flg,
        case
            when
                not isescalated
                and datediff('day', createddate, closeddate) > 7
                and datediff('day', createddate, closeddate) <= 14
                then 1
            else 0
        end as resolution_quality_good_flg,
        case when isescalated or datediff('day', createddate, closeddate) > 14 then 1 else 0 end
            as resolution_quality_poor_flg,
        case when not (
