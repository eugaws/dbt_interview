...
final as (
    select
        ...
        case when coalesce(numberofleads, 0) > 0 then coalesce(actualcost, 0.0) / numberofleads else null end as cost_per_lead,
        case when coalesce(numberofopportunities, 0) > 0 then coalesce(actualcost, 0.0) / numberofopportunities else null end as cost_per_opportunity,
        case when coalesce(numberofwonopportunities, 0) > 0 then coalesce(actualcost, 0.0) / numberofwonopportunities else null end as cost_per_won_opportunity,
        case when coalesce(numberofleads, 0) > 0 then coalesce(numberofconvertedleads, 0) * 1.0 / numberofleads else null end as lead_conversion_rate,
        case when coalesce(numberofopportunities, 0) > 0 then coalesce(numberofwonopportunities, 0) * 1.0 / numberofopportunities else null end as win_rate,
        case when coalesce(actualcost, 0.0) > 0 then (coalesce(amountwonopportunities, 0.0) - actualcost) / actualcost else null end as roi,
