...
final as (
    select
        ...
        case
            when
                upper(name) like '%NORTH%' or upper(name) like '%EASTERN%' or upper(name) like '%WESTERN%'
                then 'Regional'
            when upper(name) like '%INTERNATIONAL%' then 'International'
            when upper(name) like '%CHANNEL%' then 'Channel'
            else 'General'
        end as scope_type,
        coalesce(parentroleid is not null and parentroleid != '000000000000000AAA', false) as has_parent_role,
